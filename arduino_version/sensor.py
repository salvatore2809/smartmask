...
class Sensor:
    ...
    def read_bme680(self):
        ...
        if data is not None:  # Data was received
            char_received = data.decode()
            self.str_received += char_received
            if char_received=='\n' and len(self.str_received) > 4:
                str_num=""
                number=0
                #print(self.str_received)
                for char in self.str_received:
                    if char.isdigit() or char=='.':
                        str_num+=char
                #print(str_num, end=' ')
                if str_num.count('.') == 1:
                    number=float(str_num)
                if "gas" in self.str_received:
                    self.gas = number*1000
                    #print(str(time.monotonic())+" "+str(gas))
                    #print(" gas "+str(self.gas),end=' ')
                if "hum" in self.str_received:
                    self.hum = number
                    #print(" hum "+str(self.hum),end=' ')
                    #print(str(time.monotonic())+" "+str(gas))
                    end_reading=True
                if "press" in self.str_received:
                    self.press = number
                    #print(" press "+str(self.press),end=' ')
                    #print(str(time.monotonic())+" "+str(gas))
                if "temp" in self.str_received:
                    self.temp = number
    ...
    def get_sensor_value(self):
        try:
            ...
            if self.sensor_name == ("gas" or "in_mask_temp" or "in_mask_humidity" or "pressure") :
                #if (current_time-self.last_read_time) > 0.28:
